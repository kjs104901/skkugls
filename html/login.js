...
$("#install-group").css('display', 'none');
$("#run-group").css('display', 'none');

ipcRenderer.send("checkGLSInstalledReq", true);
ipcRenderer.on("checkGLSInstalledRes", (event, message) => {
    if (message.data) {
        $("#login-form-group").css('display', 'block');
    }
    else {
        $("#install-group").css('display', 'block');
    }
})
